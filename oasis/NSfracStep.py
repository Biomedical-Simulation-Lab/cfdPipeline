...
# Modified by Mehdi Najafi, 8-AUG-2018
...
default_solver = NS_parameters["solver"]
#print ('Loading solver: '+ default_solver)
solver = importlib.import_module('NSfracStep.'+default_solver)
...
lesmodel = importlib.import_module('.'.join(('NSfracStep.LES', les_model)))
...
if include_gravitational_effects:
   print("Adding constant gravity of 9.81E-3 m/(mili s.s)")
   f = body_force_gravity(**vars())
else:
   f = body_force(**vars())
...
#tic()
stop = False
total_timer = OasisTimer("Simulation Started:", True)
while t < (T - tstep * DOLFIN_EPS) and not stop:
    ...
    if True: #tstep % print_intermediate_info == 0:
        # Print progress
        ss = cpu_time * (T/t-1)
        hh, ss = divmod(ss, 60*60)
        mm, ss = divmod(ss, 60)
        s = "Time step %d finished in %.2f seconds, %.1f%% done (t=%8.4f, T=%g; %02d:%02d:%02d remaining)." \
            % (tstep, timestep_cpu_time, 100.0*t/T, t, T, hh, mm, ss)
        info_green (s)
