...
def get_job_env_setting():
    ...
    return job_utils.get_env_settings('SOLVER_POST_HOME') + '\n' +\
           job_utils.get_env_settings('HDF5') + '\n'

def get_cfl_job_cmd(input_folders, interval, ncores, detailed):
    cmdtxt = '\n# CFL Calculations\n'
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--time', dest='required_time', type=int, default=20,
    ...
    parser.add_argument('--detailed', dest='detailed', action='store_true', default=False,
                        help='try to print or write detailed analysis on every time step.')
