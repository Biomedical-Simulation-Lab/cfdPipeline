#!/bin/python
# ///////////////////////////////////////////////////////////////
#  Job to Calculate Volumetric Metrics: TAVEL, TKE, SPI
...
def get_hemo_job_cmd(input_folders, interval, ncores):
    ...
    for input_folder in input_folders:
        ...
        cmdtxt += '  python $SOLVER_POST_HOME/volumetric_metrics.py %s %f %d %d > %s/logs/volumetric_metrics_%s.log\n'%(input_folder,period,interval,ncores,case_folder,case_name)
    return cmdtxt

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Volumetric Metrics Calculation Job - August 2018')
    ...
    job_cmd_text = job_utils.get_job_script_header('vlm_'+'+'.join(case_names), req_time=args.required_time, nodes=1, run_on_debug=args.debug)
    ...
    job_utils.submit_a_job('volm', job_cmd_text, depend=str(args.depend) if args.depend>0 else '')
