...
class HDF5Link:
    ...
    def link(self, hdf5filename, link_from, link_to):
        ...
        use_mpiio = MPI.size(MPI.comm_world) > 1
        #self.cpp_link_module.link_dataset(MPI.comm_world, hdf5filename, link_from, link_to, use_mpiio)
        hdf5 = HDF5File(MPI.comm_world, hdf5filename, "a", use_mpiio)
        hdf5.H5Lcreate_hard(link_from, '', link_to, '')
        hdf5.close()
...
def save_hdf5(fullname, field_name, data, timestep, hdf5_link):
    ...
    local_hash = sha1()
    local_hash.update(str(data.function_space().mesh().num_cells()).encode('utf-8'))
    local_hash.update(str(data.function_space().ufl_element()).encode('utf-8'))
    local_hash.update(str(data.function_space().dim()).encode('utf-8'))
    local_hash.update(str(MPI.size(MPI.comm_world)).encode('utf-8'))

    # Global hash (same on all processes), 10 digits long
    global_hash = MPI.sum(MPI.comm_world, int(local_hash.hexdigest(), 16))
    global_hash = str(int(global_hash%1e10)).zfill(10)

    # Open HDF5File
    if not path.isfile(fullname):
        datafile = HDF5File(MPI.comm_world, fullname, 'w')
    else:
        datafile = HDF5File(MPI.comm_world, fullname, 'a')
